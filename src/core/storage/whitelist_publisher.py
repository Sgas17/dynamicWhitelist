...
class WhitelistPublisher:
    ...
    async def publish_whitelist(
        ...
        results["redis"] = await self._publish_to_redis(
            chain, whitelist, publish_metadata
        )

        # Publish to JSON backup (audit/recovery)
        results["json"] = await self._publish_to_json(
            chain, whitelist, publish_metadata
        )

        # Publish to NATS messaging (real-time consumers)
        results["nats"] = await self._publish_to_nats(
            chain, whitelist, publish_metadata
        )
    ...
    async def _publish_to_redis(
        self, chain: str, whitelist: List[Dict[str, Any]], metadata: Dict[str, Any]
    ) -> bool:
    ...
    async def _publish_to_json(
        ...
        try:
            ...
            whitelist_data = {"metadata": metadata, "whitelist": whitelist}
    ...
    async def _publish_to_nats(
        ...
        try:
            ...
            async with WhitelistNatsPublisher() as nats_publisher:
                success = await nats_publisher.publish_whitelist(
                    chain, whitelist, metadata
                )
                return success
    ...
    async def get_published_whitelist(
        self, chain: str
