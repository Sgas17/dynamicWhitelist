        ...
        try:
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    elif chain == 'ethereum' and token.get('address') and not token.get('id'):
                        missing_query = """
        ...
        try:
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    coingecko_tokens = [t for t in tokens if t.get('id') or t.get('coingecko_id')]
                    missing_tokens = [t for t in tokens if not (t.get('id') or t.get('coingecko_id')) and t.get('address')]
                    ...
                    # Handle missing platform data for other chains
                        ...
                        missing_platform_records = [
                            (
                                token['address'].lower(),
                                token.get('token_id', ''),  # May be empty for missing data
                                token.get('decimals'),
                                chain,
                                token.get('total_supply')
                            )
                        ...
                        await conn.executemany(missing_platform_query, missing_platform_records)
                        stored_count += len(missing_tokens)
