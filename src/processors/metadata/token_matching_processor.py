...
class TokenMatchingProcessor(BaseProcessor):
    ...
    async def process(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                f"Processing {len(exchange_tokens)} exchange tokens for {len(target_chains)} chains"
            )
            ...
            if not token_metadata:
                return ProcessorResult(
                    success=False, error="Failed to load CoinGecko token metadata"
                )
            ...
            self.logger.info(
                f"Successfully matched {len(matched_tokens)}/{len(exchange_tokens)} tokens"
            )
    ...
    async def _load_coingecko_metadata(
        self, target_chains: List[str]
        ...
        try:
            async with self.storage.pool.acquire() as conn:
                ...
                self.logger.info(
                    f"Loaded {total_tokens} tokens from CoinGecko database:"
                )
        ...
        for platform, tokens in token_metadata.items():
            ...
            for token_data in tokens:
                confidence, match_type = self._calculate_match_confidence(
                    exchange_symbol, mapped_symbol, token_data
                )
    ...
    def _calculate_match_confidence(
        self, exchange_symbol: str, mapped_symbol: str, token_data: Dict[str, Any]
    ) -> Tuple[float, str]:
    ...
    async def _save_results(
        self, matched_tokens: List[TokenMatch], output_file: str
    ) -> None:
    ...
    async def process_with_all_chains(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                f"Processing {len(exchange_tokens)} exchange tokens for all chains: {target_chains}"
            )
            ...
            for exchange_token in exchange_tokens:
                ...
                for match in matches:
                    if match.confidence >= min_confidence:
                        ...
                        if (
                            chain not in chain_addresses
                            or match.confidence > chain_addresses[chain]["confidence"]
                        ):
            ...
            # Calculate statistics
            total_addresses = sum(
                len(token["chain_addresses"]) for token in processed_tokens
            )
            coverage_by_chain = {}
            ...
            self.logger.info(
                f"Successfully processed {len(processed_tokens)}/{len(exchange_tokens)} tokens"
            )
            self.logger.info(f"Total chain addresses collected: {total_addresses}")
            self.logger.info(
                f"Multi-chain tokens: {len(multi_chain_tokens)}, Single-chain: {len(single_chain_tokens)}"
            )
            ...
            return ProcessorResult(
                ...
                metadata={
                    ...
                    "multi_chain_distribution": {
                        str(count): len(
                            [t for t in processed_tokens if t["chain_count"] == count]
                        )
                        for count in range(
                            1,
                            max([t["chain_count"] for t in processed_tokens], default=1)
                            + 1,
                        )
                    },
                    "min_confidence": min_confidence,
    ...
    async def _save_all_chains_results(
        self, processed_tokens: List[Dict[str, Any]], output_file: str
    ) -> None:
        ...
        export_data = {
            "metadata": {
                ...
                "total_addresses": sum(
                    len(t["chain_addresses"]) for t in processed_tokens
                ),
            },
            "tokens": [],
        }
        ...
        self.logger.info(
            f"Saved {len(processed_tokens)} tokens with all chain addresses to {output_path}"
        )
