...
from ..token_matching_processor import TokenMatch, TokenMatchingProcessor
...
class TestTokenMatchingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_token_matching_pipeline(
        self, sample_hyperliquid_tokens, mock_coingecko_data
    ):
        ...
        with patch.object(
            processor, "_load_coingecko_metadata", return_value=mock_coingecko_data
        ):
    ...
    @pytest.mark.asyncio
    async def test_multi_chain_coverage(
        self, sample_hyperliquid_tokens, mock_coingecko_data
    ):
...
class TestHyperliquidFetcherIntegration:
    ...
    async def test_mock_hyperliquid_fetch_with_processor(self):
        ...
        with patch.object(
            ...
            with patch.object(
                processor, "_load_coingecko_metadata", return_value=mock_db_data
            ):
...
class TestRealHyperliquidIntegration:
    ...
    async def test_real_hyperliquid_fetch(self):
        ...
        """
        pytest.skip(
            "Skipping real API test - run manually with: pytest -m integration -k real_hyperliquid_fetch"
        )
        ...
        try:
            ...
            if result.success:
                ...
                for match in matches[:3]:
                    print(
                        f"  {match.symbol} → {match.chain}: {match.chain_address[:10]}..."
                    )
