...
from src.core.orchestrator import Task, TaskOrchestrator
        ...
        async with StorageManager() as storage:
            ...
            transfers_task = Task(
                ...
                parameters={
                    ...
                    "processor_params": {"hours_back": 24, "min_transfers": 50},
                    "chain": "ethereum",
                },
            )
            ...
            for task_id, result in results["results"].items():
                ...
                if result.success and hasattr(result, "output") and result.output:
                    if "processor_type" in result.output:
                        ...
                        print(
                            f"      Processed: {result.output.get('processed_count', 0)} items"
                        )
    ...
    try:
        ...
        async with StorageManager() as storage:
            ...
            fallback_task = Task(
                ...
                task_type="process",
                parameters={"input_data": {"test": "data", "count": 5}},
            )
