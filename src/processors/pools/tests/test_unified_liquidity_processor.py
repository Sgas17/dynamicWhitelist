...
V4_TEST_POOL = (
    "0xdce6394339af00981949f5f3baf27e3610c76326a700af57e4b3e3ae4977f78d".lower()
)  # V4 pool ID
...
class TestStatistics:
    ...
    async def test_snapshot_statistics(self, db_engine):
        ...
        logger.info(
            f"Average bitmap words per pool: {stats['avg_bitmap_words_per_pool']:.2f}"
        )
...
async def test_end_to_end_v3_and_v4(db_engine, v3_liquidity_data, v4_liquidity_data):
    ...
    if v3_snapshot:
        ...
        logger.info(
            f"  Ticks: {v3_snapshot['total_ticks']}, Block: {v3_snapshot['snapshot_block']}"
        )

    if v4_snapshot:
        logger.info(f"✓ V4 snapshot exists: {V4_TEST_POOL}")
        logger.info(
            f"  Ticks: {v4_snapshot['total_ticks']}, Block: {v4_snapshot['snapshot_block']}"
        )
...
async def test_liquidity_snapshot_validation(db_engine):
    ...
    # ============================================================================
    logger.info(
        "Step 0: Fetching up-to-date liquidity events from production fetcher..."
    )
    ...
    if not v3_fetch_result.success:
        raise RuntimeError(
            f"Failed to fetch V3 test pool events: {v3_fetch_result.error}"
        )
    ...
    if not v4_fetch_result.success:
        raise RuntimeError(
            f"Failed to fetch V4 test pool events: {v4_fetch_result.error}"
        )
    ...
    if not v3_snapshot:
        logger.warning(
            "⚠ V3 snapshot not found - test pool may not have events in the data range"
        )
        ...
        logger.info(
            f"  Ticks with liquidity in snapshot: {v3_snapshot['total_ticks']}\n"
        )

        # Verify snapshot is at target block
        if v3_snapshot["snapshot_block"] != target_block:
            logger.error(
                f"❌ ERROR: Snapshot at block {v3_snapshot['snapshot_block']} but target is {target_block}!"
            )
            ...
            TICK_CHUNK_SIZE = 500
            total_chunks = (
                len(all_valid_ticks) + TICK_CHUNK_SIZE - 1
            ) // TICK_CHUNK_SIZE
            logger.info(
            ...
            if failed_chunks > 0:
                logger.warning(
                    f"  ⚠ {failed_chunks}/{total_chunks} chunks failed to fetch"
                )
            ...
            for tick in all_valid_ticks:
                ...
                snap_gross = (
                    snapshot_tick_data.get("liquidity_gross", 0)
                    if snapshot_tick_data
                    else 0
                )
                snap_net = (
                    snapshot_tick_data.get("liquidity_net", 0)
                    if snapshot_tick_data
                    else 0
                )
            ...
            if mismatches:
                ...
                for i, mismatch in enumerate(mismatches[:10]):  # Show first 10
                    logger.warning(
                        f"  {i + 1}. Tick {mismatch['tick']}: {mismatch['issue']}"
                    )
            ...
            match_percentage = (
                (matches / total_compared * 100) if total_compared > 0 else 0
            )
            ...
            if v3_validation_passed:
                logger.info(
                    "✓ V3 validation PASSED: Snapshot matches live state perfectly!\n"
                )
            else:
                logger.warning(
                    f"⚠ V3 validation FAILED: {len(mismatches)} mismatches found\n"
                )
    ...
    if not v4_snapshot:
        logger.warning(
            "⚠ V4 snapshot not found - test pool may not have events in the data range"
        )
        ...
        # Generate all valid ticks based on tick_spacing
        all_valid_ticks_v4 = list(
            range(
                MIN_TICK_V4
                - (MIN_TICK_V4 % V4_TICK_SPACING),  # Round to nearest valid tick
                MAX_TICK_V4 + 1,
                V4_TICK_SPACING,
            )
        )
        ...
        logger.info(
            f"  Ticks with liquidity in snapshot: {v4_snapshot['total_ticks']}\n"
        )
        ...
        total_chunks_v4 = (
            len(all_valid_ticks_v4) + TICK_CHUNK_SIZE_V4 - 1
        ) // TICK_CHUNK_SIZE_V4
        logger.info(
            f"  Fetching {len(all_valid_ticks_v4)} ticks in {total_chunks_v4} chunks of {TICK_CHUNK_SIZE_V4}"
        )
        ...
        for chunk_idx in range(0, len(all_valid_ticks_v4), TICK_CHUNK_SIZE_V4):
            ...
            if not batch_result.success:
                logger.warning(
                    f"  ⚠ Chunk {chunk_num}/{total_chunks_v4} failed: {batch_result.error}"
                )
                failed_chunks_v4 += 1
                continue
        ...
        if failed_chunks_v4 > 0:
            logger.warning(
                f"  ⚠ {failed_chunks_v4}/{total_chunks_v4} chunks failed to fetch"
            )
        ...
        if len(live_tick_data_v4) == 0:
            ...
            logger.info(
                f"  Successfully fetched {total_chunks_v4 - failed_chunks_v4}/{total_chunks_v4} chunks\n"
            )
            ...
            if v4_validation_passed:
                logger.info(
                    "✓ V4 validation PASSED: Snapshot matches live state perfectly!\n"
                )
            else:
                logger.warning(
                    f"⚠ V4 validation FAILED: {len(mismatches)} mismatches found\n"
                )
    ...
    if v3_validation_passed is not None:
        logger.info(
            f"  Validation: {'✓ PASSED' if v3_validation_passed else '✗ FAILED'}"
        )
    else:
    ...
    if v4_validation_passed is not None:
        logger.info(
            f"  Validation: {'✓ PASSED' if v4_validation_passed else '✗ FAILED'}"
        )
    else:
        logger.info(f"  Validation: SKIPPED (no snapshot)")
    logger.info("=" * 80 + "\n")
    ...
    if v3_validation_passed is False or v4_validation_passed is False:
        pytest.fail(
            "Validation failed: Snapshot state does not match live on-chain state"
        )
