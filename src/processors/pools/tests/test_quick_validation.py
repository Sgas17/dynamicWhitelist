...
async def test_quick_validation():
    ...
    all_valid_ticks = list(range(
        MIN_TICK - (MIN_TICK % V3_TICK_SPACING),
        MAX_TICK + 1,
        V3_TICK_SPACING
    ))
    ...
    logger.info(f"  ⚠ CRITICAL: Fetching at snapshot block {v3_snapshot['snapshot_block']}\n")
    ...
    assert match_percentage >= 99.9, f"Validation failed: only {match_percentage:.2f}% match (expected >= 99.9%)"
