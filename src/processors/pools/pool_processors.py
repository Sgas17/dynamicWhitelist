...
class UniswapV3PoolProcessor(BaseProcessor):
    ...
    def __init__(self, chain: str = "ethereum"):
        ...
        self.factory_addresses = self.config.protocols.get_factory_addresses(
            "uniswap_v3", chain
        )
        self._pool_manager_addresses = [
            HexBytes(addr) for addr in self.factory_addresses
        ]
    ...
    async def process(
        self, start_block: int = 0, events_path: Optional[str] = None
    ) -> ProcessorResult:
        """
        ...
        try:
            ...
            if not events_path:
                events_path = (
                    data_dir / self.chain / f"{self.protocol}_poolcreated_events"
                )
            else:
    ...
    async def _process_events(
        self, events_path: Path, start_block: int
    ) -> List[Dict[str, Any]]:
...
class UniswapV4PoolProcessor(BaseProcessor):
    ...
    def __init__(self, chain: str = "ethereum"):
        ...
        self._pool_manager_addresses = [
            HexBytes(addr) for addr in self.pool_manager_addresses
        ]
    ...
    def validate_config(self) -> bool:
        ...
        if not self.pool_manager_addresses:
            self.logger.warning(
                f"No pool manager addresses configured for chain: {self.chain}"
            )
            return False

        self.logger.info(
            f"Using {len(self.pool_manager_addresses)} pool manager addresses for V4"
        )
        return True
    ...
    async def process(
        ...
        try:
            ...
            if not events_path:
                events_path = (
                    data_dir / self.chain / f"{self.protocol}_initialize_events"
                )
            else:
                events_path = Path(events_path)
...
class UniswapV2PoolProcessor(BaseProcessor):
    ...
    def validate_config(self) -> bool:
        ...
        self.logger.info(
            f"Using {len(self.factory_addresses)} factory addresses for V2"
        )
        return True
    ...
    async def process(
        ...
        try:
            ...
            if not events_path:
                events_path = (
                    data_dir / self.chain / f"{self.protocol}_paircreated_events"
                )
            else:
                events_path = Path(events_path)
...
class AerodromeV2PoolProcessor(BaseProcessor):
    ...
    def __init__(self, chain: str = "base"):
        ...
        self._factory_address = (
            HexBytes(self.factory_address) if self.factory_address else None
        )
    ...
    def validate_config(self) -> bool:
        ...
        if not self.factory_address:
            self.logger.error(
                f"No factory address for Aerodrome V2 on chain: {self.chain}"
            )
            return False
    ...
    def _process_single_event(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "chain": self.chain,
                "additional_data": {"pair_index": pair_index, "stable": stable},
            }
...
class AerodromeV3PoolProcessor(BaseProcessor):
    ...
    def validate_config(self) -> bool:
        ...
        if not self.factory_address:
            self.logger.error(
                f"No factory address for Aerodrome V3 on chain: {self.chain}"
            )
            return False
