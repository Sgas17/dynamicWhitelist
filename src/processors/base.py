...
class BaseProcessor(ABC):
    ...
    def _setup_blacklist(
        self, blacklist_file: Optional[str] = None, auto_update: bool = False
    ):
        """
    ...
    def _filter_blacklisted_pools(
        ...
        for pool in pools:
            ...
            for asset_key in ["asset0", "asset1", "asset2", "asset3"]:
                ...
                if token_address and self.blacklist_manager.is_blacklisted(
                    token_address
                ):
            ...
            if blacklisted_tokens:
                ...
                pool["blacklist_reason"] = (
                    f"Contains blacklisted tokens: {', '.join(blacklisted_tokens)}"
                )
                filtered_pools.append(pool)
                self.logger.info(
                    f"Filtered pool {pool['address']}: {pool['blacklist_reason']}"
                )
            else:
    ...
    async def _get_last_processed_block(self) -> int:
        ...
        try:
            ...
            async with self.storage.pool.acquire() as conn:
                ...
                last_block = (
                    result["max_block"] if result and result["max_block"] else 0
                )

                self.logger.info(
                    f"Last processed block for {self.protocol}: {last_block}"
                )
                return last_block
    ...
    async def _store_pools_to_database(self, pools: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            count = await self.storage.store_pools_batch(
                pools, self.chain, self.protocol
            )
            self.logger.info(
                f"Stored {count} pools to database for {self.chain}/{self.protocol}"
            )
            return count
    ...
    def _deduplicate_pools(self, pools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.warning(
            "_deduplicate_pools is deprecated, database handles deduplication"
        )
        return pools
