...
class TestV4DirectStateView:
    ...
    def encode_function_call(self, function_signature: str, args: list) -> str:
        ...
        param_types = (
            function_signature[param_start:param_end].split(",")
    ...
    @pytest.mark.asyncio
    async def test_get_slot0_direct(
        self, web3_connection, state_view_address, test_pool_id
    ):
        ...
        result = web3_connection.eth.call(
            {"to": state_view_address, "data": "0x" + call_data, "gas": 1000000}
        )
        ...
        sqrt_price_x96, tick, protocol_fee, lp_fee = decode(
            ["uint160", "int24", "uint24", "uint24"], result
        )
    ...
    @pytest.mark.asyncio
    async def test_get_tick_bitmap_direct(
        self, web3_connection, state_view_address, test_pool_id
    ):
        ...
        for word_pos in test_words:
            print(f"\n   📍 Word {word_pos}:")
            call_data = self.encode_function_call(
                "getTickBitmap(bytes32,int16)", [pool_id_bytes, word_pos]
            )
            ...
            if bitmap > 0:
                ...
                if sample_bits:
                    sample_ticks = [
                        ((word_pos << 8) + bit_pos) * 60 for bit_pos in sample_bits
                    ]
