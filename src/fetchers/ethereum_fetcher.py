...
class EthereumFetcher(CryoFetcher):
    ...
    async def fetch_uniswap_v2_pools(
        self, output_dir: Optional[str] = None, from_checkpoint: bool = True
    ) -> FetchResult:
        """
        ...
        try:
            ...
            deployment_block = self.protocol_config.get_deployment_block(
                "uniswap_v2", "ethereum"
            )
            factory_contracts = self.protocol_config.get_factory_addresses(
                "uniswap_v2", "ethereum"
            )

            # Get event hash from config
            pair_created_event = self.protocol_config.get_event_hash(
            ...
            if from_checkpoint:
                checkpoint_block = await self._get_last_processed_block(
                    "uniswap_v2_pools", output_dir
                )
                if checkpoint_block:
            ...
            self.logger.info(
                f"Fetching Uniswap V2 pools from block {start_block} to {latest_block}"
            )
                ...
                result.metadata.update(
                    {
                        ...
                        "checkpoint_used": from_checkpoint
    ...
    async def fetch_uniswap_v3_pools(
        ...
        try:
            ...
            # Get event hash from config
            pool_created_event = self.protocol_config.get_event_hash(
            ...
            self.logger.info(
                f"Fetching Uniswap V3 pools from block {start_block} to {latest_block}"
            )
    ...
    async def fetch_uniswap_v4_pools(
        ...
        try:
            ...
            pool_manager_contracts = self.protocol_config.get_factory_addresses(
                "uniswap_v4", "ethereum"
            )

            # Get event hash from config
            initialized_event = self.protocol_config.get_event_hash(
            ...
            self.logger.info(
                f"Fetching Uniswap V4 pools from block {start_block} to {latest_block}"
            )
    ...
    async def fetch_recent_transfers(
        self, hours_back: int = 24, output_dir: Optional[str] = None
    ) -> FetchResult:
        """
    ...
    async def fetch_liquidity_events(
        ...
        try:
            ...
            self.logger.info(
                f"Fetching {protocol} liquidity events from block {start_block} to {end_block}"
            )
    ...
    async def _get_last_processed_block(
        self, event_type: str, output_dir: Optional[str] = None
    ) -> Optional[int]:
        """
    ...
    async def fetch_uniswap_v4_liquidity_events(
        ...
        try:
            ...
            # Get event hash from config
            modify_liquidity_event = self.protocol_config.get_event_hash(
            ...
            if from_checkpoint:
                ...
                checkpoint_block = await self._get_last_processed_block(
                    "uniswap_v4_liquidity", output_dir
                )
                if checkpoint_block:
                    start_block = checkpoint_block
                    ...
                    start_block, end_block = await self.calculate_block_range(
                        hours_back
                    )
            ...
            self.logger.info(
                f"Fetching Uniswap V4 liquidity events from block {start_block} to {end_block}"
            )
