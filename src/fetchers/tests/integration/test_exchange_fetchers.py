...
class TestHyperliquidIntegration:
    ...
    async def test_fetch_real_hyperliquid_swap_markets(self):
        ...
        try:
            ...
            assert len(tokens) == filtered_tokens, "Token list should match filtered count"
            ...
            for i, token in enumerate(tokens[:10]):
                print(f"  {i+1:2d}. {token.symbol:12s} | {token.base:8s} | Active: {token.is_active}")
            ...
            assert len(found_majors) > 0, f"Should find at least one major pair from {major_pairs}"
        ...
        try:
            ...
            for token in tokens:
                ...
                if token.quote not in acceptable_quotes:
                    print(f"Warning: Unexpected quote asset {token.quote} for {token.symbol}")
...
class TestExchangeFetchersIntegration:
    ...
    async def test_compare_exchange_implementations(self):
        ...
        for exchange, metadata in results.items():
            ...
            print(f"{exchange:12s}: {metadata.get('total_markets', 0):4d} markets, {len(tokens):4d} tokens")
