...
class TestCryoFetcher:
    ...
    async def test_fetch_logs_success(
        self, mock_glob, mock_mkdir, mock_run, mock_subprocess, cryo_fetcher
    ):
    ...
    @patch("subprocess.run")
    @pytest.mark.asyncio
    async def test_fetch_logs_validation_failure(
        self, mock_run, mock_subprocess, cryo_fetcher
    ):
    ...
    @patch("subprocess.run")
    @pytest.mark.asyncio
    async def test_fetch_logs_cryo_failure(
        self, mock_run, mock_subprocess, cryo_fetcher
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_transfers(
        self, mock_fetch_logs, mock_get_latest_block, cryo_fetcher
    ):
    ...
    @patch.object(CryoFetcher, "fetch_logs")
    @pytest.mark.asyncio
    async def test_fetch_pool_events(
        self, mock_fetch_logs, mock_get_latest_block, cryo_fetcher
    ):
