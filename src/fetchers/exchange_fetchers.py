...
class BaseExchangeFetcher:
    ...
    async def fetch_markets(self, market_type: str = "swap") -> FetchResult:
        ...
        """
        try:
            self.logger.info(f"Fetching {market_type} markets from {self.exchange_id}...")
            ...
            if market_type == "swap":
                markets = await loop.run_in_executor(
                    None, 
                    self.ccxt_exchange.fetchSwapMarkets
                )
            elif market_type == "spot":
                all_markets = await loop.run_in_executor(
                    None,
                    self.ccxt_exchange.fetchMarkets
                )
            ...
            self.logger.info(f"Fetched {len(tokens)} tokens from {len(markets)} {self.exchange_id} markets")
