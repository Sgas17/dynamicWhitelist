...
async def test_whitelist_builder():
    ...
    try:
        ...
        # Check required keys
        required_keys = [
            'all_tokens', 'all_pairs', 'decimals', 'symbols',
            'v2_pairs', 'v3_pairs', 'v4_pairs', 'whitelist'
        ]
        ...
        for pool_addr, pair_data in sample_pairs:
            print(f"\n{pool_addr}:")
            print(f"  token0: {pair_data['token0']['symbol']} ({pair_data['token0']['address'][:10]}...)")
            print(f"  token1: {pair_data['token1']['symbol']} ({pair_data['token1']['address'][:10]}...)")
        ...
        if result['v2_pairs']:
            ...
            for addr, pair in sample_v2:
                print(f"    {addr[:10]}... - {pair['exchange']} - {pair['token0']['symbol']}/{pair['token1']['symbol']}")
        ...
        if 'token_sources' in result:
            ...
            for token, sources in sorted(multi_source.items(), key=lambda x: -len(x[1]))[:5]:
        ...
        sample_output = {
            ...
            'sample_tokens': {k: result['all_tokens'][k] for k in list(result['all_tokens'].keys())[:5]},
            'sample_pairs': {k: result['all_pairs'][k] for k in list(result['all_pairs'].keys())[:5]},
            'sample_decimals': {k: result['decimals'][k] for k in list(result['decimals'].keys())[:5]},
            'sample_symbols': {k: result['symbols'][k] for k in list(result['symbols'].keys())[:5]},
