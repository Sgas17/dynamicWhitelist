...
class ChainConfig(BaseConfig):
    ...
    MIN_LIQUIDITY_V2: float = BaseConfig.get_env_float("MIN_LIQUIDITY_V2", 2000.0)  # $2k
    MIN_LIQUIDITY_V3: float = BaseConfig.get_env_float("MIN_LIQUIDITY_V3", 2000.0)  # $2k
    MIN_LIQUIDITY_V4: float = BaseConfig.get_env_float("MIN_LIQUIDITY_V4", 1000.0)  # $1k

    TRUSTED_TOKENS: List[str] = field(default_factory=lambda: ["weth", "usdc", "usdt", "dai", "wbtc", "usde"]) 
    ...
    def get_trusted_tokens_for_chain(self, ) -> Dict[str, Dict[str, str]]:
    ...
    def get_chain_trusted_tokens(self, chain_name: str) -> Dict[str, hex]:
        ...
        return [token for token in self.TRUSTED_TOKENS if token in self.get_trusted_tokens_for_chain()[chain_name]]
