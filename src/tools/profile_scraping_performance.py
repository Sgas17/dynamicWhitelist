...
async def profile_v2_scraping(
    ...
    metrics = {
        ...
        "recommended_batch_size_12s": int(pools_per_12_seconds * 0.8)  # 80% safety margin
    }

    logger.info(f"V2 Performance: {pools_per_second:.2f} pools/sec, "
...
async def profile_v3_scraping(
    ...
    logger.info(f"V3 (slot0) Performance: {pools_per_second:.2f} pools/sec, "
                f"recommended batch size: {metrics['recommended_batch_size_12s']}")
...
async def profile_v4_scraping(
    ...
    logger.info(f"V4 (slot0) Performance: {pools_per_second:.2f} pools/sec, "
                f"recommended batch size: {metrics['recommended_batch_size_12s']}")
...
async def profile_full_tick_scraping(
    ...
    """
    logger.info(f"Profiling full tick data scraping ({sample_size_per_protocol} pools per protocol)...")
    ...
    if v3_results:
        ...
        logger.info(f"V3 (full ticks) Performance: {pools_per_second:.2f} pools/sec, "
    ...
    if v4_results:
        ...
        logger.info(f"V4 (full ticks) Performance: {pools_per_second:.2f} pools/sec, "
...
async def main():
    ...
    try:
        ...
        full_tick_metrics = await profile_full_tick_scraping(reth_loader, storage, sample_size_per_protocol=20)
        ...
        logger.info(f"  V2: {v2_metrics.get('pools_per_second', 0):.2f} pools/sec → "
                    f"Batch size: {v2_metrics.get('recommended_batch_size_12s', 0)}")
        logger.info(f"  V3: {v3_metrics.get('pools_per_second', 0):.2f} pools/sec → "
                    f"Batch size: {v3_metrics.get('recommended_batch_size_12s', 0)}")
        logger.info(f"  V4: {v4_metrics.get('pools_per_second', 0):.2f} pools/sec → "
        ...
        if 'v3_full_ticks' in full_tick_metrics:
            ...
            logger.info(f"  V3: {v3_full.get('pools_per_second', 0):.2f} pools/sec → "
        ...
        if 'v4_full_ticks' in full_tick_metrics:
            ...
            logger.info(f"  V4: {v4_full.get('pools_per_second', 0):.2f} pools/sec → "
                        f"Batch size: {v4_full.get('recommended_batch_size_12s', 0)}")

        logger.info("\nRecommendations for 12-second block time:")
        logger.info(f"  - V2 filtering: {v2_metrics.get('recommended_batch_size_12s', 0)} pools per batch")
        logger.info(f"  - V3 filtering: {v3_metrics.get('recommended_batch_size_12s', 0)} pools per batch")
        logger.info(f"  - V4 filtering: {v4_metrics.get('recommended_batch_size_12s', 0)} pools per batch")
        if 'v3_full_ticks' in full_tick_metrics:
            logger.info(f"  - V3 full ticks: {full_tick_metrics['v3_full_ticks'].get('recommended_batch_size_12s', 0)} pools per batch")
        if 'v4_full_ticks' in full_tick_metrics:
            logger.info(f"  - V4 full ticks: {full_tick_metrics['v4_full_ticks'].get('recommended_batch_size_12s', 0)} pools per batch")
