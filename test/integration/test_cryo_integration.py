    ...
    async def test_fetch_recent_transfers_real(self, cryo_fetcher, temp_output_dir):
        ...
        try:
            ...
            if result.success:
                ...
                assert "rate" in result.error.lower() or "timeout" in result.error.lower()
...
class TestEthereumFetcherIntegration:
    ...
    def test_calculate_block_range_realistic(self, ethereum_fetcher):
        """Test block range calculation produces realistic ranges."""
        start_block, end_block = ethereum_fetcher.calculate_block_range(1)  # 1 hour
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_fetch_uniswap_v3_pools_sample(self, ethereum_fetcher, temp_output_dir):
        ...
        try:
            ...
            if result.success:
                ...
                assert result.fetched_blocks > 1000000  # Millions of blocks since deployment
                assert Path(result.data_path).exists()
